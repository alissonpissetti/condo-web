...
import { CondominiumMaintenancesApiService } from '../../../core/condominium-maintenances-api.service';
import {
...
import type { MaintenanceTimelineAttachment } from '../../../core/condominium-maintenances-api.service';
import { ObrasAttachmentFileIconComponent } from '../painel-obras/obras-attachment-file-icon.component';
import { ObrasAttachmentPreviewModalService } from '../painel-obras/obras-attachment-preview-modal.service';
...
@Component({
  selector: 'app-manutencoes-timeline-attachment-preview',
  standalone: true,
  imports: [ObrasAttachmentFileIconComponent],
  templateUrl: '../painel-obras/obras-timeline-attachment-preview.component.html',
  styleUrl: '../painel-obras/obras-timeline-attachment-preview.component.scss',
})
export class ManutencoesTimelineAttachmentPreviewComponent
  implements OnInit, AfterViewInit, OnDestroy
{
  readonly condominiumId = input.required<string>();
  readonly maintenanceId = input.required<string>();
  readonly entryId = input.required<string>();
  readonly attachment = input.required<MaintenanceTimelineAttachment>();
  ...
  protected readonly replaceInputId = `manut-att-replace-${Math.random().toString(36).slice(2)}`;
  ...
  private loadMediaBlob(
    att: MaintenanceTimelineAttachment,
    kind: 'audio',
  ): void {
    this.state.set('loading');
    this.api
